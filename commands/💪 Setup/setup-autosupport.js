    ...
    try {
      ...
      async function first_layer() {
        ...
        let menumsg = await message.reply({
          embeds: [MenuEmbed],
          components: [new MessageActionRow().addComponents(Selection), new MessageActionRow().addComponents(new MessageButton().setStyle("LINK").setURL("https://youtu.be/QGESDc31d4U").setLabel("Tutorial Video").setEmoji("1433092796360822784"))]
        })
        ...
        collector.on('collect', menu => {
          if (menu?.user.id === cmduser.id) {
            ...
            content: `<:no:1431196812349542440>You are not allowed to do that! Only: <@${cmduser.id}>`,
            ephemeral: true
          });
        });
        ...
        collector.on('end', collected => {
          menumsg.edit({
            ...
            components: [],
            content: `<:yes:1431196867844374569> **Selected: \`${collected && collected.first() && collected.first().values ? collected.first().values[0] : "Nothing"}\`**`
          })
        });
      }
      async function second_layer() {
        ...
        collector.on('end', collected => {
          menumsg.edit({
            embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
            components: [],
            content: `<:yes:1431196867844374569> **Selected: \`${collected ? collected.first().values[0] : "Nothing"}\`**`
          })
        });
      }
      async function handle_the_picks(optionhandletype, menuoptiondata) {
        switch (optionhandletype) {
          case "Send the Config	Message": {
            ...
            if (collected && collected.first().content) {
              ...
              if (collected2 && collected2.first().mentions.channels.size > 0) {
                ...
                return message.reply("<:no:1431196812349542440> **You did not ping a valid Channel!**")
              }
            } else {
              return message.reply("<:no:1431196812349542440> **You did not enter a Valid Message in Time! CANCELLED!**")
            }
          }
          break;
          case "Add AutoSup Option": {
            let data = theDB.get(message.guild.id, "data");
            if (data.length >= 25) {
              return message.reply("<:no:1431196812349542440> **You reached the limit of 25 different Options!** Remove another Option first!")
            }
            ...
            if (collected && collected.first().content) {
              if (!collected.first().content.includes("++")) return message.reply("<:no:1431196812349542440> **Invalid Usage! Please mind the Usage and check the Example**")
              let value = collected.first().content.split("++")[0].trim().substr(0, 25);
              let index2 = data.findIndex(v => v.value == value);
              if(index2 >= 0 && index != index2) {
                  return message.reply("<:no:1431196812349542440> **Options can't have the SAME VALUE!** There is already an Option with that Value!");
              }
          ...
          case "Edit AutoSup Option": {
            let data = theDB.get(message.guild.id, "data");
            if (!data || data.length < 1) {
              return message.reply("<:no:1431196812349542440> **There are no Open-Ticket-Options to edit**")
            }
            let embed = new MessageEmbed()
              .setColor(es.color)
              .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
              ...
              if (menu?.user.id === cmduser.id) {
                ...
                collector.on('end', collected => {
                  tempmsg.edit({
                    embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)],
                    components: [],
                    content: `<:yes:1431196867844374569> **Selected: \`${collected ? collected.customId : "Nothing | CANCELLED"}\`**`
                  })
                });
                } else {
                  return message.reply("<:no:1431196812349542440> **You did not enter a Valid Message in Time! CANCELLED!**")
          ...
          let data = theDB.get(message.guild.id, "data");
          if (!data || data.length < 1) {
            return message.reply("<:yes:1431196867844374569> **There are no Auto-Responding-Support-Options to remove**")
          }
          let embed = new MessageEmbed()
            .setColor(es.color)
            .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
