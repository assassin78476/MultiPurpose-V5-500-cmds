    ...
    if (message.author.id != "1222599088126689292")
      return message.channel.send({
        embeds: [new MessageEmbed()
          ...
          .setTitle("Only Bot Owner is allowed to deploy the SLASH-COMMANDS")
          .setDescription(`Go to the [Discord-Server](https://discord.gg/a7r25Cz2UD), open a Ticket and ask for it!`)
        ]
      });
    try {
      ...
      if(loadSlashsGlobal){
        let themsg = await message.reply(`<a:Loading:1433023454587654245> **Attempting to set the Global Slash Commands in \`${client.guilds.cache.size} Guilds\`...**`)
        ...
        let guild = client.guilds.cache.get(guildId);
        let themsg = await message.reply(`<a:Loading:1433023454587654245> **Attempting to set the GUILD Slash Commands in \`${guild.name}\`...**`)
